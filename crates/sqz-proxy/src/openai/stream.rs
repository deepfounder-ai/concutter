...
/// Convert a streaming upstream response into an axum SSE response.
///
/// The byte stream from reqwest is forwarded as-is to the client with the
/// appropriate SSE headers set. No parsing or modification of individual
/// events takes place -- the proxy acts as a transparent pipe for streaming
/// completions.
